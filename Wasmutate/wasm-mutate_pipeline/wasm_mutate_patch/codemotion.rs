...
// ---------------------------------------------------------------------
// Single-purpose variants of CodemotionMutator, added so each code-motion
// sub-mutation is independently selectable via `WasmMutate::categories`
// (see MutationCategory in lib.rs). These reuse CodemotionMutator's own
// `random_mutate` helper (which doesn't touch `self`, so any
// `CodemotionMutator` value works as the receiver) with a single-element
// mutators list, instead of CodemotionMutator::mutate's own list of both
// IfComplementMutator and LoopUnrollMutator. CodemotionMutator itself is
// left completely unchanged above, so default/unfiltered behavior
// (categories left empty) is bit-for-bit identical to before this patch.
// ---------------------------------------------------------------------

/// Only applies the if/else-branch-complement code motion mutation
/// (maps to MutationCategory::ConditionalSwap).
#[derive(Clone, Copy)]
pub struct IfSwapMutator;

impl Mutator for IfSwapMutator {
    fn mutate<'a>(
        &self,
        config: &mut WasmMutate<'a>,
    ) -> Result<Box<dyn Iterator<Item = Result<Module>> + 'a>> {
        let mutators: Vec<Box<dyn AstMutator>> = vec![Box::new(IfComplementMutator)];
        CodemotionMutator.codemotion_mutate_with(config, &mutators)
    }
...
/// Only applies the loop-unrolling code motion mutation (maps to
/// MutationCategory::LoopUnrolling). Named distinctly from
/// `loop_unrolling::LoopUnrollMutator` (the inner AstMutator this wraps)
/// to avoid a name collision.
#[derive(Clone, Copy)]
pub struct LoopUnrollOnlyMutator;

impl Mutator for LoopUnrollOnlyMutator {
    fn mutate<'a>(
        &self,
        config: &mut WasmMutate<'a>,
    ) -> Result<Box<dyn Iterator<Item = Result<Module>> + 'a>> {
        let mutators: Vec<Box<dyn AstMutator>> = vec![Box::new(LoopUnrollMutator)];
        CodemotionMutator.codemotion_mutate_with(config, &mutators)
    }
...
impl CodemotionMutator {
    /// Shared tail of the original `mutate()` body, factored out so
    /// `IfSwapMutator`/`LoopUnrollOnlyMutator` can reuse it with their own
    /// single-element `mutators` list instead of duplicating the
    /// code-section-rebuild logic.
    fn codemotion_mutate_with<'a>(
        &self,
        config: &mut WasmMutate<'a>,
        mutators: &[Box<dyn AstMutator>],
    ) -> Result<Box<dyn Iterator<Item = Result<Module>> + 'a>> {
