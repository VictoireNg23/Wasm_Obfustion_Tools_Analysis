...
ORIG_WASM_DIR="./realworld_program"
MUT_WASM_DIR="./WasmMutate/WasmMutate_Results/realworld_program"
RESULTS_DIR="./WasmMutate/WasmMutate_Results/realworld_program/Results_realworld_program"
CSV_FILE="./WasmMutate/WasmMutate_Results/Results_Csv/Results_Spectec_Wasmutate_Wasmtime/realworld_program.csv"
LOG_FILE="$MUT_WASM_DIR/Log_wasmtime/realworld_program.log"

MUTATOR="./WasmMutate/wasm_mutator_by_category/target/release/wasm_mutator_by_category"
NODE_PUPPET="./WasmMutate/run_browser_test.js"
...
# Si CSV n'existe pas → créer header
if [[ ! -f "$CSV_FILE" ]]; then
...
already_done() {
    ...
    # mutant = mut1 / mut2 / mut3 ...
    grep -q "^$sample,.*,$combo,$mutant," "$CSV_FILE"
}
...
# METRICS (inchangé)
# ==========================================================
validate_wasm() { wasm-tools validate "$1" >/dev/null 2>&1 && echo "yes" || echo "no"; }
count_call_indirect() { wasm-tools print "$1" 2>/dev/null | grep -c "call_indirect"; }
...
disassembly_ok() { wasm2wat "$1" >/dev/null 2>&1 && echo "yes" || echo "no"; }
...
cfg_similarity() {
    ...
    python3 - <<EOF
A=set(open("$d/a").read().splitlines()); B=set(open("$d/b").read().splitlines())
print("{:.2f}".format((len(A&B)/len(A|B))*100 if A|B else 0))
EOF
    rm -rf "$d"
}

count_func_symbols() { wasm-objdump -x "$1" 2>/dev/null | grep "Export.*func" | wc -l; }
count_type_symbols() { wasm-objdump -x "$1" 2>/dev/null | grep "^Type" | wc -l; }
...
process_wasm_file() {
    ...
    for combo in "${ALL_COMBOS[@]}"; do
        for ((i=1;i<=NB_MUTANTS;i++)); do
            ...
            # >>>>>>>>>>>>>>> REPRISE AUTOMATIQUE <<<<<<<<<<<<<<<<<
            if already_done "$sample" "$combo" "$mutant_id"; then
                log "SKIP $sample | $combo | $mutant_id already in CSV"
                continue
            fi

            outdir="$MUT_WASM_DIR/$sample/$combo/$mutant_id"
            mkdir -p "$outdir"

            obf_start=$(date +%s%3N)
            "$MUTATOR" --input "$wasm" --categories "${combo//_/,}" --variants 1 --outdir "$outdir" >/dev/null 2>&1 || true
            obf_end=$(date +%s%3N)
            ...
            printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,yes,yes,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" \
            "$sample" "$wasm" "$combo" "$mutant_id" \
            ...
            log "OK $sample | $combo | $mutant_id | run_obf=$run_obf | obf_time=${obf_time}ms"
        done
    done
}
