...
# cfg_similarity.py
"""
Structural CFG similarity.

IMPORTANT FIX: the original version of this script used
`wasm-opt --print-cfg`, which is NOT a real Binaryen flag (verified against
the WebAssembly/binaryen source tree and against `wasm-opt 108`, i.e. the
version installed by `apt install binaryen` on Ubuntu 22.04 -- it errors
with "Unknown option '--print-cfg'"). That made cfg_similarity_structural()
always return None, silently, for every row.

This version builds the CFG itself from `wasm2wat` (WABT) disassembly text
via cfg_from_wat.py -- no dependency on any nonstandard/nonexistent tool
flag, stable across WABT/Binaryen versions. Everything downstream (node
signatures, Hungarian-matching similarity score) is unchanged.

Methodology:
- For every basic-block node (across all functions in the module, pooled
  together), build a structural signature:
      (bfs_depth_from_entry, in_degree, out_degree)
  rather than comparing raw block labels/instructions -- so it is
  unaffected by identifier-renaming transformations.
- Find an optimal bipartite matching between the orig and obf signature
  multisets that minimizes total signature distance (Hungarian algorithm).
