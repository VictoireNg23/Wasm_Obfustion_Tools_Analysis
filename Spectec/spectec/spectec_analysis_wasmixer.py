...
python3 spectec_analysis_wasmixer.py \
    ...
    --input-csv  /path/to/wasmixer_results.csv \
    --output-csv /path/to/wasmixer_results_spectec.csv \
    --obf-dir    /path/to/wasmixer_outdir \
...
def main():
    ...
    ap.add_argument("--dataset", required=True,
                     help="The ORIGINAL dataset root -- must match run_wasmixer.py's own --dataset "
                          "exactly, since relpath_orig in the CSV is relative to it")
    ap.add_argument("--input-csv", required=True, help="WasMixer metrics CSV to enrich")
    ap.add_argument("--output-csv", required=True, help="Enriched CSV output path (resumable)")
    ap.add_argument("--obf-dir", required=True, help="run_wasmixer.py's --outdir")
