...
"""
Worker: process_one
- tmp copy, run wasmixer, validate, native run (wasmtime), browser run (puppeteer via node)
...
def has_start(wat_text):
    if not wat_text: return False
    return ("(start" in wat_text) or bool(re.search(r'\(export\s+"(_start|main)"', wat_text))
...
def wasm_cfg_text(path):
    rc, out, err = run_cmd(["wasm-opt", "--print-cfg", str(path)])
    return out if rc == 0 else None
...
def process_one(sample_rel, wasm_src_path, out_root, wasmixer_cli, opts_list, wabt_bins, timeout, tmp_root):
    ...
    cfg_orig = wasm_cfg_text(tmp_wasm)
    ...
    # native run original (wasmtime)
    runtime_orig_log = out_combo_dir / "runtime_orig.log"
    if wabt_bins.get("wasmtime") and wat_text and has_start(wat_text):
        exports = re.findall(r'\(export\s+"([^"]+)"\s+\(func', wat_text)
        if "main" in exports:
            cmd = [wabt_bins["wasmtime"], "--invoke", "main", str(tmp_wasm)]
        elif exports:
            cmd = [wabt_bins["wasmtime"], "--invoke", exports[0], str(tmp_wasm)]
        else:
            cmd = [wabt_bins["wasmtime"], str(tmp_wasm)]
        t0 = time.time()
        rc_run, out_run, err_run = run_cmd(cmd, timeout)
        t1 = time.time()
        runtime_orig_log.write_text((out_run or "") + "\n" + (err_run or ""))
        summary["run_orig"] = "ok" if rc_run == 0 else ("timeout" if rc_run == -1 else f"err:{rc_run}")
        if rc_run >= 0:
            summary["run_time_orig"] = round(t1 - t0, 6)
    else:
        runtime_orig_log.write_text("no_entry\n")
        summary["run_orig"] = "no_entry"
    ...
    summary["cfg_similarity"] = wat_similarity(cfg_orig, cfg_obf)

    # native run obf
    runtime_obf_log = out_combo_dir / "runtime_obf.log"
    if wabt_bins.get("wasmtime") and wat_obf_text and has_start(wat_obf_text):
        exports2 = re.findall(r'\(export\s+"([^"]+)"\s+\(func', wat_obf_text)
        if "main" in exports2:
            cmd = [wabt_bins["wasmtime"], "--invoke", "main", str(out_obf)]
        elif exports2:
            cmd = [wabt_bins["wasmtime"], "--invoke", exports2[0], str(out_obf)]
        else:
            cmd = [wabt_bins["wasmtime"], str(out_obf)]
        t0 = time.time()
        rc_run2, out_run2, err_run2 = run_cmd(cmd, timeout)
        t1 = time.time()
        runtime_obf_log.write_text((out_run2 or "") + "\n" + (err_run2 or ""))
        summary["run_obf"] = "ok" if rc_run2 == 0 else ("timeout" if rc_run2 == -1 else f"err:{rc_run2}")
        if rc_run2 >= 0:
            summary["run_time_obf"] = round(t1 - t0, 6)
    else:
        runtime_obf_log.write_text("no_entry\n")
        summary["run_obf"] = "no_entry"
